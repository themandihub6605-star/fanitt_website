...
export function TrustBar() {
  ...
  return (
    <section ref={parallaxRef} className="border-y border-white/10 bg-navy-800/45 py-8">
      <Container>
        <motion.div style={{ y }} className="grid grid-cols-2 gap-y-5 divide-white/10 sm:grid-cols-4 sm:gap-y-0 sm:divide-x">
          {items.map((stat, i) => (
...
function StatItem({
  ...
  return (
    <div className="text-center">
      <p className="bg-gradient-to-r from-orange-400 via-pink-400 to-yellow-300 bg-clip-text font-display text-2xl font-bold tracking-tight text-transparent sm:text-3xl">
        {stat.prefix}
        {stat.display(count)}
        {stat.suffix}
      </p>
