...
export default function AccountSuspended() {
  ...
  useEffect(() => {
    // Set once by apiClient.ts's interceptor right before it redirects here
    // — sessionStorage (not Redux) because auth state gets cleared in the
    // same interceptor call, and reading it once on mount is all this page
    // needs.
    const stored = sessionStorage.getItem('fanitt_suspension_message');
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#0A0A0A] px-6 text-center">
      ...
      <motion.div
        ...
        className="w-full max-w-md rounded-[2rem] border border-red-500/20 bg-navy-800/60 p-8 shadow-lifted"
      >
        ...
        <p className="mt-3 text-sm leading-relaxed text-white/60">
          An admin has restricted access to your Fanitt account, so you can't log in or use the platform right now.
        </p>
        ...
        <Link to="/" className="mt-4 block text-sm font-semibold text-white/50 hover:text-white/80">
          Back to home
        </Link>
      </motion.div>
    </div>
