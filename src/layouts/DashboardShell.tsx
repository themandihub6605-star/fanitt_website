  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] lg:flex">
      ...
      <aside className="hidden w-64 shrink-0 flex-col border-r border-white/10 bg-navy-800/40 px-4 py-6 lg:flex">
        ...
        <Link
          to="/"
          className="mb-4 flex items-center gap-3 rounded-xl border border-white/10 px-3 py-2.5 text-sm font-semibold text-white/70 transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-orange-400/40 hover:bg-white/5 hover:text-white"
        >
          <Globe size={18} />
          Visit Website
        </Link>
          ...
          {NAV.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.href}
                className={cn(
                  'group relative flex items-center gap-3 overflow-hidden rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-200 ease-out',
                  active ? 'bg-orange-500/15 text-orange-300' : 'text-white/60 hover:translate-x-0.5 hover:bg-white/5 hover:text-white'
                )}
              >
                {active && (
                  <motion.span
                    layoutId="sidebar-active-pill"
                    className="absolute inset-y-1 left-0 w-[3px] rounded-full bg-gradient-to-b from-orange-400 to-pink-500"
                    transition={{ type: 'spring', stiffness: 380, damping: 32 }}
                  />
                )}
                <item.icon size={18} className="transition-transform duration-200 ease-out group-hover:scale-110" />
                <span className="flex-1">{item.label}</span>
                {!!item.badge && (
                  <span className="flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white animate-pulse">
        ...
        <Link
          to="/pricing"
          className="group mt-6 block overflow-hidden rounded-2xl border border-orange-500/20 bg-gradient-to-br from-orange-500/10 to-pink-500/10 p-4 transition-all duration-300 ease-out hover:-translate-y-0.5 hover:border-orange-400/50 hover:shadow-glow"
        >
          <div className="flex items-center gap-2 text-orange-300">
            <Crown size={16} className="transition-transform duration-300 ease-out group-hover:rotate-12" />
          ...
          <form onSubmit={handleSearch} className="hidden max-w-md flex-1 lg:block">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-full border border-white/10 bg-navy-800/60 py-2.5 pl-10 pr-4 text-sm text-white placeholder:text-white/30 transition-all duration-200 ease-out focus:border-orange-400 focus:ring-4 focus:ring-orange-400/15"
              />
            </div>
          </form>
          ...
          <div className="ml-auto flex shrink-0 items-center gap-1.5 sm:gap-2.5">
            {isCreator ? (
              <Link
                ...
                className="hidden items-center gap-1.5 rounded-full border border-red-500/30 px-3.5 py-2 text-xs font-bold text-red-400 transition-all duration-200 ease-out hover:-translate-y-0.5 hover:bg-red-500/10 hover:shadow-card sm:flex"
              >
              ...
              <Link
                to="/campaigns/new"
                className="hidden items-center gap-1.5 rounded-full border border-orange-500/30 px-3.5 py-2 text-xs font-bold text-orange-400 transition-all duration-200 ease-out hover:-translate-y-0.5 hover:bg-orange-500/10 hover:shadow-card sm:flex"
              >
            ...
            {walletBalance !== null && (
              <Link
                to="/wallet"
                className="flex shrink-0 items-center gap-1 rounded-full border border-white/10 bg-navy-800/60 px-2.5 py-2 text-xs font-bold text-yellow-300 transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-yellow-400/40 hover:shadow-card sm:gap-1.5 sm:px-3"
              >
