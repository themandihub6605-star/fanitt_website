    ...
    <>
      <nav className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-around border-t border-white/10 bg-[#0A0A0A]/95 px-2 pb-[env(safe-area-inset-bottom)] pt-1.5 backdrop-blur-xl lg:hidden">
        {TABS.map((tab) => {
          ...
          if (tab.isCreate) {
            const createButton = (
              <span className="flex h-12 w-12 items-center justify-center rounded-full text-white shadow-glow bg-orange-500 transition-transform duration-200 ease-out active:scale-90">
          ...
          if (tab.isExplore) {
            return (
              <button
                ...
                className={cn('flex flex-col items-center gap-1 px-3 py-2 text-[11px] font-medium transition-colors duration-200', active ? 'text-orange-400' : 'text-white/50')}
              >
                <span className={cn('inline-flex items-center justify-center rounded-full p-1.5 transition-all duration-300 ease-out', active && 'bg-orange-500/15 -translate-y-0.5')}>
          ...
          if (tab.isProfile && (user?.role === 'creator' || user?.role === 'brand')) {
            return (
              <button
                ...
                className={cn('flex flex-col items-center gap-1 px-3 py-2 text-[11px] font-medium transition-colors duration-200', active ? 'text-orange-400' : 'text-white/50')}
              >
                <span className={cn('relative inline-flex items-center justify-center rounded-full p-1.5 transition-all duration-300 ease-out', active && 'bg-orange-500/15 -translate-y-0.5')}>
                  <tab.icon size={20} strokeWidth={active ? 2.4 : 2} />
          ...
          return (
            ...
            >
              <span
                className={cn(
                  'relative inline-flex items-center justify-center rounded-full p-1.5 transition-all duration-300 ease-out',
                  active && 'bg-orange-500/15 -translate-y-0.5'
                )}
              >
                <tab.icon size={20} strokeWidth={active ? 2.4 : 2} />
                {tab.isProfile && isAuthenticated && <span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full border-2 border-[#0A0A0A] bg-emerald-400" />}
              </span>
              {tab.label}
            </Link>
          );
