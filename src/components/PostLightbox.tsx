...
export function PostLightbox({ posts, index, onClose, onIndexChange }: PostLightboxProps) {
  ...
  // Which media item within the *current* post is showing — resets to 0
  // whenever we move to a different post.
  ...
  const goToPost = (newIndex: number) => {
    setMediaIndex(0);
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/* between-post navigation (outer arrows, further from center) */}
        {index > 0 && (
          <button
            onClick={(e) => {
              e.stopPropagation();
          ...
          {activeMedia.type === 'video' ? (
            <div className="relative">
              <video
                key={activeMedia.url}
                ref={videoRef}
                src={getUploadUrl(activeMedia.url)}
            ...
            <img src={getUploadUrl(activeMedia.url)} alt={post.caption} className="max-h-[85vh] w-full object-contain" />
          )}

          {/* within-post navigation, only when this post has >1 media item */}
          {mediaItems.length > 1 && (
            <>
              {mediaIndex > 0 && (
                <button
