...
import { LayoutDashboard, LogOut, ChevronDown, CalendarCheck, FileText } from 'lucide-react';
...
export function UserMenu() {
  ...
  const dashboardHref = user.role === 'creator' ? '/dashboard/creator' : user.role === 'brand' ? '/dashboard/brand' : null;

  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen((v) => !v)} className="flex items-center gap-2 rounded-full py-1 pl-1 pr-2.5 hover:bg-white/10">
        {user.avatarUrl ? (
        ...
        <span className="max-w-[100px] truncate text-sm font-semibold text-white/80">{user.name}</span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 mt-3 w-56 overflow-hidden rounded-2xl border border-white/10 bg-[#0d1120] shadow-lifted"
          >
